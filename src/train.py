...
# optimizer = optim.Adam(model.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=0.0005)
...
for e in range(epoch_n):
    ...
    for i, data in enumerate(trainloader):
        ...
        crop_x = (label.shape[-1] - pred.shape[-1]) // 2
        crop_y = (label.shape[-2] - pred.shape[-2]) // 2
        label = label[:, :, crop_y: label.shape[-2] - crop_y, crop_x: label.shape[-1] - crop_x]
...
with torch.no_grad():
    for i in range(testset.__len__()):
        ...
        output_mask = torch.max(pred, dim=1)[1].squeeze(0).cpu().numpy()
...
for i in range(testset.__len__()):
  axes[i, 0].imshow(output_labels[i][0])
  axes[i, 0].axis('off')
  axes[i, 1].imshow(output_masks[i])
  axes[i, 1].axis('off')

fig.savefig('test_result.png', dpi=1000, bbox_inches='tight')